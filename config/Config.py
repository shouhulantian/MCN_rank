...
class Config(object):
    def __init__(self, args):
        ...
        self.test_epoch = 1
        self.pretrain_model = './checkpoint/checkpoint_Bert'
    ...
    def get_train_batch(self):
        ...
        for b in range(self.train_batches):
            ...
            #print(sen t_lengths, sent_lengths.size())
            ...
            yield {'context_idxs': context_idxs[:cur_bsz, :max_c_len].contiguous(),
                   ...
                   'sent_idxs': sent_idxs[:cur_bsz].contiguous(),
                   'sent_lengths': sent_lengths[:cur_bsz].contiguous(),
                   'reverse_sent_idxs': reverse_sent_idxs[:cur_bsz, :max_c_len].contiguous(),
                            ...
                            for r in range(1, self.relation_num):
                                ...
                                if two_phase:
                                    if is_rel_exist[i,j,1] > is_rel_exist[i,j,0]:
                                        test_result.append( ((h_idx, t_idx, str(r)) in label, float(predict_re[i,j,r]), intrain,  titles[i], self.id2rel[str(r)], index, h_idx, t_idx, r) )
                                    else:
                                        test_result.append( ((h_idx, t_idx, str(r)) in label, -100.0, intrain,  titles[i], self.id2rel[str(r)], index, h_idx, t_idx, r) )
                                else:
                                    test_result.append( ((h_idx, t_idx, str(r)) in label, float(predict_re[i,j,r]), intrain,  titles[i], self.id2rel[str(r)], index, h_idx, t_idx, r) )
