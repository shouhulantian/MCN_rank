...
class GraphATTConfig(object):
    def __init__(self, args):
        ...
        self.test_epoch = 3
        self.pretrain_model = 'checkpoint/checkpoint_GCN_bert_ematt_woSS'
        #self.pretrain_model = 'checkpoint/checkpoint_BiLSTM'
        self.word_size = 100
            ...
            for i, index in enumerate(cur_batch):
                ...
                for m in range(men_count):
                    for n in range(men_count):
                        ...
                        elif abs(mention_loc[n]-mention_loc[m]) <=1:
                            edge_dis[i, L + m, L + n] = 1.0  # MM
                            edge_dis[i, L + n, L + m] = 1.0
    ...
    def train(self, model_pattern, model_name):
        ...
        # for i in ori_model.bi.parameters():
    ...
    def test(self, model, model_name, output=False, input_theta=-1):
        ...
        if output:
            ...
            output = [{'index': x[-4], 'h_idx': x[-3], 't_idx': x[-2], 'r_idx': x[-1], 'r': x[-5], 'title': x[-6],'predict_re':x[-8]} for x
                      in test_result[:w + 1]]
