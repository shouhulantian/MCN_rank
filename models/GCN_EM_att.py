...
from .EM_attention import Attention
...
class GCN_ATT(nn.Module):
    def __init__(self, config):
        ...
        # for i in self.linear_re.parameters():
        #     i.requires_grad = False
        ...
        # for i in self.gcn1.parameters():
        #     i.requires_grad = False
        self.gcn2 = GraphConvolution(hidden_size,hidden_size,bias=True)
        # for i in self.gcn2.parameters():
        #     i.requires_grad = False
        #self.att = Attention(hidden_size*2,config.relation_num)

        if self.use_distance:
            self.dis_emb = nn.Embedding(20, config.dis_size, padding_idx=10)
            hidden_size = hidden_size*2 + config.dis_size

        self.bi = torch.nn.Bilinear(hidden_size, hidden_size, config.relation_num)
        #self.rel_li = nn.Linear(hidden_size*2, config.relation_num)
    ...
    def forward(self, context_idxs, pos, context_ner, context_lens,
                ...
                node_type,context_masks,context_starts,dis_h_2_t, dis_t_2_h,mention_count,relation_label,is_train):
        ...
        context_output = torch.matmul(node_mapping,context_output)

        if self.use_node_type:
            context_output = torch.cat([context_output,self.node_embed(node_type)],dim=-1)

        node_feature = self.gcn1(context_output,edge_weight)
        node_feature = self.gcn2(node_feature, edge_weight)
        #node_feature = self.dropout(node_feature)
        # node_feature = self.gcn2(node_feature,edge_weight)
        node_cat = torch.cat([node_feature,context_output],dim=-1)
        head_gcn = self.entity_pair_construct(head_index,node_cat)
        tail_gcn = self.entity_pair_construct(tail_index,node_cat)
        head_node = torch.cat([head_gcn, self.dis_emb(dis_h_2_t)], dim=-1)
        tail_node = torch.cat([tail_gcn, self.dis_emb(dis_t_2_h)], dim=-1)
        # head_bert = self.entity_pair_construct(head_index,context_output)
        # tail_bert = self.entity_pair_construct(tail_index,context_output)

        # gcn_pair = torch.cat([head_gcn,tail_gcn],dim=-1)
        # bert_pair = torch.cat([head_bert,tail_bert],dim=-1)
        #predict_re = self.rel_li(torch.cat([head_node,tail_node],dim=-1) )
        predict_re = self.bi(head_node, tail_node)
        #predict_re = self.att(gcn_pair,bert_pair,relation_label,is_train)

        return predict_re

        # if is_train:
        #     if self.use_distance:
        #         head_node = torch.cat([head_att.squeeze(2), self.dis_emb(dis_h_2_t)], dim=-1)
        #
        #     predict_re = self.bi(head_node, tail_node)
        # else:
        #     dis_h_2_t = dis_h_2_t.unsqueeze(-1).repeat(1, 1, self.config.relation_num)
        #     dis_t_2_h = dis_t_2_h.unsqueeze(-1).repeat(1, 1, self.config.relation_num)
        #     if self.use_distance:
        #         head_node = torch.cat([head_att, self.dis_emb(dis_h_2_t)], dim=-1)
        #         tail_node = torch.cat([tail_att, self.dis_emb(dis_t_2_h)], dim=-1)
        #     predict_re = self.bi(head_node,tail_node)
